...
def _self_loop_degrees(adjacency: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """Combinatorial ``d_i`` and self-loop-augmented ``dtilde_i = d_i + 1``.

    ``dtilde`` is the degree of the renormalized graph ``Wtilde = W + I`` whose
    symmetric-normalized Laplacian ``L = I - A_hat`` (self loops) the screened
    metric ``M_tau = L + tau I`` is built on; the ``+ 1`` is the unit self loop.
    """

    d = _degrees(adjacency)
    return d, d + 1.0


def _edge_weight_lookup(
    adjacency: torch.Tensor, edge_i: torch.Tensor, edge_j: torch.Tensor
) -> torch.Tensor:
    """Vectorized ``w_ab`` for each ``(edge_i, edge_j)`` from a sparse adjacency."""

    adj = adjacency.coalesce()
    idx, val = adj.indices(), adj.values()
    n = adjacency.shape[0]
    key = idx[0].to(torch.int64) * n + idx[1].to(torch.int64)
    order = torch.argsort(key)
    key_sorted, val_sorted = key[order], val[order]
    query = edge_i.to(torch.int64) * n + edge_j.to(torch.int64)
    pos = torch.searchsorted(key_sorted, query).clamp_max(
        max(key_sorted.numel() - 1, 0)
    )
    hit = key_sorted[pos] == query
    return torch.where(hit, val_sorted[pos], torch.zeros_like(val_sorted[pos]))


def screened_symmetric_edge_costs(
    adjacency: torch.Tensor,
    a0: torch.Tensor,
    edge_i: torch.Tensor,
    edge_j: torch.Tensor,
    ...
    *,
    ...
    r"""Exact single-level local-variation **edge** cost for the screened symmetric
    Laplacian ``M_tau = (I - A_hat) + tau I`` (working note *From Subspace to Edge
    Costs*, Thm. 1)::

        cost_tau(e) = ell_tau(a, b) * || abar_a - abar_b ||^2 ,

    with the degree-corrected embedding ``abar_i = a0[i] / sqrt(dtilde_i)`` (rows of
    ``Dtilde^{-1/2} a0``), self-loop-augmented degree ``dtilde_i = d_i + 1``, and
    the three-part prefactor (edge Dirichlet + doubled cut + tau x Ward)::

        ell_tau(a, b) = w_ab
            + 2[(d_a - w_ab) dtilde_b^2 + (d_b - w_ab) dtilde_a^2] / (dtilde_a + dtilde_b)^2
            + tau * dtilde_a * dtilde_b / (dtilde_a + dtilde_b) .

    ``a0`` MUST be the ``M_tau``-orthonormal basis ``a0 = V (V^T M_tau V)^{-1/2}``.
    On a regular graph at ``tau = 0`` this reduces to Loukas' combinatorial edge
    weight ``(d_a + d_b) / 2``. ``edge_weight`` supplies ``w_ab`` per edge (looked
    up from ``adjacency`` when ``None``).
    """

    eps = torch.finfo(a0.dtype).eps
    d, dt = _self_loop_degrees(adjacency)
    if edge_weight is None:
        edge_weight = _edge_weight_lookup(adjacency, edge_i, edge_j)
    edge_weight = edge_weight.to(a0.dtype)
    da, db = d[edge_i], d[edge_j]
    dta, dtb = dt[edge_i], dt[edge_j]
    denom = (dta + dtb).clamp_min(eps)
    dirichlet = edge_weight
    cut = (
        2.0
        * ((da - edge_weight) * dtb.square() + (db - edge_weight) * dta.square())
        / denom.square()
    )
    ward = tau * (dta * dtb) / denom
    ell = (dirichlet + cut + ward).clamp_min(0.0)
    abar = a0 / dt.clamp_min(eps).sqrt().unsqueeze(1)
    diff_sq = (abar[edge_i] - abar[edge_j]).square().sum(dim=1)
    return (ell * diff_sq).clamp_min(0.0)


def _loukas_edge_costs(
    adjacency: torch.Tensor,
    A: torch.Tensor,
    edge_i: torch.Tensor,
    edge_j: torch.Tensor,
    tau: float,
    *,
    cost_model: str = "combinatorial",
    edge_weight: torch.Tensor | None = None,
) -> torch.Tensor:
    """Per-edge local-variation cost dispatcher.

    ``cost_model="combinatorial"`` (default, legacy) returns Loukas' combinatorial
    edge cost ``1/4 (d_a + d_b)^2 ||A_a - A_b||^4`` (``+ tau`` screening); on a
    combinatorial ``L``-orthonormal ``A`` this is the original recipe. ``"symmetric"``
    returns the exact screened symmetric-Laplacian cost of eq. edgecost
    (:func:`screened_symmetric_edge_costs`), and then ``A`` must be the
    ``M_tau``-orthonormal basis.
    """

    if cost_model == "symmetric":
        return screened_symmetric_edge_costs(
            adjacency, A, edge_i, edge_j, tau, edge_weight=edge_weight
        )
...
def _edge_partition(
    ...
    *,
    ...
    costs = _loukas_edge_costs(
        ...
        edge_weight=adjacency.values()[upper],
    )
    ...
    for i in range(n):
        ...
        candidate_costs.append(
            _local_variation_cost(
                members, A, degree, neighbors, weight, eps, tau, cost_model=cost_model
            )
        )
...
def _local_variation_cost(
    ...
    ``cost_model="symmetric"`` instead returns the exact screened symmetric-Laplacian
    set cost (working note *From Subspace to Edge Costs*, generalizing Thm. 1 to
    ``|C| > 1``): the degree-weighted deflation ``v_C = Dtilde_C^{1/2} 1_C / sqrt(vol C)``
    on the self-loop-augmented degrees ``dtilde_i = d_i + 1``, the *doubled* cut of
    the local Laplacian ``M_{tau,C} = Dtilde^{-1/2} L_C Dtilde^{-1/2} + tau I_C``, and
    the ``tau`` mass term -- reducing to ``ell_tau(a,b) ||abar_a - abar_b||^2`` for an
    edge.  ``A`` must then be the ``M_tau``-orthonormal basis.
    """
    ...
    if cost_model == "symmetric":
        # Screened symmetric-Laplacian local-variation cost. ``local_laplacian``
        # holds only the *internal* edges (diagonal = internal degree); the cut to
        # outside C is added separately as the doubled boundary term of M_{tau,C}.
        dt_C = d_C + 1.0  # dtilde_i = d_i + 1 (self-loop augmented)
        vol = dt_C.sum().clamp_min(eps)
        v = (dt_C.sqrt() / vol.sqrt()).unsqueeze(1)  # v_C = Dtilde^{1/2} 1 / sqrt(vol)
        y = A_C - v * (v * A_C).sum(dim=0, keepdim=True)  # Pi_C^perp A0
        yhat = y * dt_C.clamp_min(eps).rsqrt().unsqueeze(1)  # y_i / sqrt(dtilde_i)
        dirichlet = torch.trace(yhat.T @ (local_laplacian @ yhat))  # internal edges
        intra = local_laplacian.diagonal()  # sum_{j in C} w_ij (internal degree)
        cut_i = (d_C - intra).clamp_min(0.0)  # external weight to outside C
        cut = 2.0 * (cut_i.unsqueeze(1) * yhat.square()).sum()  # doubled cut
        energy = dirichlet + cut
        if tau:
            energy = energy + tau * y.square().sum()  # tau mass term (raw residual)
...
def loukas_coarsen_pytorch(
    ...
    * ``"ward"`` -- global, non-greedy contiguity-constrained Ward agglomeration
    ...
    # The local-variation edge/set costs are metric-specific: the combinatorial
    # recipe (Loukas) for ``L = D - W``, and the exact screened symmetric-Laplacian
    # cost ``ell_tau(a,b) ||abar_a - abar_b||^2`` (working note Thm. 1) for the
    # symmetric ``M_tau``.  ``cost_model`` selects which the greedy families use.
    cost_model = (
        "symmetric"
        if laplacian in ("symmetric", "normalized", "sym", "norm")
        else "combinatorial"
    )
    ...
    if method == "edges":
        partition = partial(
            _edge_partition, laplacian_fn=laplacian_fn, tau=tau, cost_model=cost_model
        )
    elif method == "neighborhood":
        partition = partial(
            _neighborhood_partition,
